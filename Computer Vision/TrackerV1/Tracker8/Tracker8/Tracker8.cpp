...
int main() {
	...
	while (charCheckForEscKey != 27 && capWebcam.isOpened()) {		// until the Esc key is pressed or webcam connection is lost
		...
		cv::inRange(matProcessed,				// funcion input
			cv::Scalar(175, 175, 175),				// min filtering value (if greater than or equal to this) (in BGR format)
			cv::Scalar(255, 255, 255),			// max filtering value (and if less than this) (in BGR format)
			matProcessed);						// function output
		...
		cv::HoughCircles(matProcessed,			// input image
			...
			5,									// min circle radius (any circles with smaller radius will not be returned) //original 10
			400);								// max circle radius (any circles with larger radius will not be returned)

//		for (int i = 0; i < v3fCircles.size(); i++) {		// for each circle . . .
		for (int i = 0, ilen = v3fCircles.size(); i < ilen; i++) {		// for each circle . . .
